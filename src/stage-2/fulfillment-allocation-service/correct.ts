...
export class FulfillmentAllocationService {
  ...
  async execute(input: {
    ...
    for (const item of order.items) {
      ...
      for (const candidate of candidates) {
        ...
        const currentShipmentWeight = existingShipment?.totalWeightKg ?? 0;

        const remainingCarrierWeight =
          candidate.carrier.maxWeightKg - currentShipmentWeight;

        const maxQuantityByWeight = Math.floor(
          remainingCarrierWeight / item.unitWeightKg
        );

        if (maxQuantityByWeight <= 0) {
          continue;
        }
  ...
  private buildCandidates(input: {
    ...
    for (const batch of input.batches) {
      ...
      if (!warehouse.supportedRegions.includes(input.order.destinationRegion)) {
        continue;
      }
  ...
  private resolveUnfulfilledReason(input: {
    ...
    const hasCarrier = warehouseEligibleBatches.some(batch => {
      ...
      return carriers.some(carrier => {
        ...
        const shipmentKey = `${batch.warehouseId}:${carrier.id}`;
        const currentShipmentWeight =
          input.shipmentMap.get(shipmentKey)?.totalWeightKg ?? 0;

        const remainingCarrierWeight =
          carrier.maxWeightKg - currentShipmentWeight;

        return remainingCarrierWeight >= input.unitWeightKg;
      });
    });
