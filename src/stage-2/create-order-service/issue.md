...
Por exemplo, considere um produto com estoque igual a `5`. Caso o pedido seja criado com os seguintes itens:

- produto `PROD-1`, quantidade `3`;
- produto `PROD-1`, quantidade `3`.

Cada item individualmente possui quantidade menor ou igual ao estoque disponível. Porém, a quantidade total solicitada para o produto é `6`, ultrapassando o estoque real de `5`. O sistema aceita os itens e permite a finalização da compra.
...
Quando a quantidade total solicitada de um produto for maior que seu estoque disponível, o serviço deve lançar o erro: `Insufficient stock`. Nesses casos, o pedido não deve ser salvo e o evento `order.created` não deve ser publicado.
...
O serviço deve rejeitar o pedido com o erro `Insufficient stock`, pois a quantidade total solicitada para `PROD-1` é `6`, excedendo o estoque disponível de `5`.