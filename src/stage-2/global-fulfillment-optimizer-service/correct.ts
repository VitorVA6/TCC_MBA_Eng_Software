    ...
    const dfs = (
      ...
      if (index === input.demandUnits.length) {
        const evaluated = this.evaluatePlan({
          ...
          choices,
          edges: input.edges,
          allWarehouseIds: input.allWarehouseIds,
          activeWarehouseIds: input.activeWarehouseIds
        });
  ...
  private findCheapestRoute(input: {
    ...
    while (queue.length > 0) {
      ...
      const nextEdges = input.edges
        .filter(edge => {
          if (edge.fromNode !== current.node) {
            return false;
          }

          if (edge.maxWeightKg < input.weight) {
            return false;
          }

          if (current.path.includes(edge.toNode)) {
            return false;
          }

          if (
            input.allWarehouseIds.has(edge.toNode) &&
            !input.activeWarehouseIds.has(edge.toNode) &&
            edge.toNode !== input.toNode
          ) {
            return false;
          }
