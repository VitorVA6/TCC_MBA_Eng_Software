// mutation1.ts
// MUTATION: Refunds are not capped by item gross.
// This allows refunds to reduce an item below zero.
...
export class MarketplaceSettlementService {
  ...
  async execute(input: {
    ...
    for (const item of items) {
      ...
      // MUTATION: should be Math.min(refundTotal, gross)
      const effectiveRefund = refundTotal;
