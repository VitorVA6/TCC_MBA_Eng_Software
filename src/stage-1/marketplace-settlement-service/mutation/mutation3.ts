// mutation3.ts
// MUTATION: Commission is calculated from gross value instead of net value after refunds.
...
export class MarketplaceSettlementService {
  ...
  async execute(input: {
    ...
    for (const item of items) {
      ...
      // MUTATION: should use netValue, not gross
      settlement.commission += gross * (commissionRate / 100);
    }
