// mutation2.ts
// MUTATION: Shipping is split by gross item value instead of net item value after refunds.
...
export class MarketplaceSettlementService {
  ...
  async execute(input: {
    ...
    const grossValueByItemId = new Map<string, number>();
    ...
    for (const item of items) {
      ...
      grossValueByItemId.set(item.id, gross);
    ...
    for (const order of orders) {
      ...
      // MUTATION: should use netValueByItemId, not grossValueByItemId
      const totalGrossInOrder = orderItems.reduce(
        (sum, item) => sum + (grossValueByItemId.get(item.id) ?? 0),
        0
      );

      if (totalGrossInOrder <= 0) {
        continue;
      }

      const grossBySellerInOrder = new Map<string, number>();

      for (const item of orderItems) {
        const current = grossBySellerInOrder.get(item.sellerId) ?? 0;
        grossBySellerInOrder.set(
          item.sellerId,
          current + (grossValueByItemId.get(item.id) ?? 0)
        );
      }

      for (const [sellerId, sellerGross] of grossBySellerInOrder.entries()) {
        const settlement = settlementBySeller.get(sellerId)!;
        settlement.shippingShare +=
          order.shippingAmount * (sellerGross / totalGrossInOrder);
      }
