...
export class GlobalFulfillmentOptimizerService {
  ...
  for (const unit of input.demandUnits) {
    const possibleWarehouses = Array.from(remainingStock.keys())
    ...
    let bestChoice: AllocationChoice | null = null;
    ...
    for (const warehouseId of possibleWarehouses) {
      ...
      if (
        route.cost < bestCost ||
        (route.cost === bestCost && warehouseId < (bestChoice?.warehouseId ?? ''))
      ) {
        bestCost = route.cost;
        bestChoice = {
    ...
    if (bestChoice) {
      const stockKey = `${bestChoice.productId}:${bestChoice.warehouseId}`;
      const available = remainingStock.get(stockKey) ?? 0;

      remainingStock.set(stockKey, available - 1);
  ...
  private findCheapestRoute(input: {
    ...
    while (queue.length > 0) {
      ...
      for (const edge of nextEdges) {
        ...
        if (nextDeliveryDays > input.maxDeliveryDays) {
          continue;
        }
