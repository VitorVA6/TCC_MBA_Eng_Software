...
export class GlobalFulfillmentOptimizerService {
  ...
  private findCheapestRoute(input: {
    ...
    const direct = input.edges
      .filter(edge =>
        edge.fromNode === input.fromNode &&
        edge.toNode === input.toNode &&
        edge.maxWeightKg >= input.weight &&
        edge.deliveryDays <= input.maxDeliveryDays
      )
      .sort((a, b) => {
        const aCost = a.fixedCost + a.costPerKg * input.weight;
        const bCost = b.fixedCost + b.costPerKg * input.weight;
    ...
    return {
      path: [input.fromNode, input.toNode],
      cost: round2(direct.fixedCost + direct.costPerKg * input.weight),
      deliveryDays: direct.deliveryDays
    };
  }
