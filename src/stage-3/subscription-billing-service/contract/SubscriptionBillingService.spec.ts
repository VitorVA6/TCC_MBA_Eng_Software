...
describe('SubscriptionBillingService', () => {
  ...
  it('4. Deve cobrar o valor proporcional da diferença no upgrade', async () => {
    setupMocks({
    ...
    expect(paymentGateway.charge).toHaveBeenCalledWith('u1', 50);
  });
  ...
  it('9. Deve buscar/aplicar cupom antes de calcular impostos', async () => {
    setupMocks({
    ...
    expect(couponRepository.findByCode).toHaveBeenCalledWith('DISC20');
    expect(taxService.getRate).toHaveBeenCalledWith('BR');

    const couponCallOrder =
      couponRepository.findByCode.mock.invocationCallOrder[0];

    const taxCallOrder =
      taxService.getRate.mock.invocationCallOrder[0];

    expect(couponCallOrder).toBeLessThan(taxCallOrder);
  });
  ...
  it('14. Deve lidar com upgrade com cupom e imposto', async () => {
    ...
    expect(result.amount).toBe(43.2);
    expect(paymentGateway.charge).toHaveBeenCalledWith('u1', 43.2);
  });
