...
describe('GlobalFulfillmentOptimizerService', () => {
  ...
  it('5. Deve ignorar estoque de armazém inativo mesmo quando há estoque e rota válida', async () => {
    orderRepository.findById.mockResolvedValue(createOrder());

    stockRepository.getStock.mockResolvedValue([
      createStock({
        warehouseId: 'W_INACTIVE',
        productId: 'P1',
        quantity: 10
      })
    ]);
  ...
  it('6. Deve ignorar posições de estoque com quantidade zero mesmo quando o armazém e a rota são válidos', async () => {
    orderRepository.findById.mockResolvedValue(createOrder());
  ...
  it('9. Deve limitar o envio ao maxWeightKg da rota e deixar o restante não atendido', async () => {
    orderRepository.findById.mockResolvedValue(createOrder({
      items: [{ productId: 'P1', quantity: 10, unitWeightKg: 5 }]
    }));

    stockRepository.getStock.mockResolvedValue([
      createStock({ warehouseId: 'W1', productId: 'P1', quantity: 10 })
    ]);
    ...
    expect(result.shipments[0].totalWeightKg).toBe(40);
  ...
  it('9b. Deve ignorar rota quando uma única unidade excede o maxWeightKg', async () => {
    orderRepository.findById.mockResolvedValue(createOrder({
      items: [{ productId: 'P1', quantity: 1, unitWeightKg: 50 }]
    }));
