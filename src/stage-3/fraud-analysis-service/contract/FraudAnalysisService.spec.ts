...
describe('FraudAnalysisService', () => {
  ...
  it('13. Deve gravar audit log com a decisão APPROVED', async () => {
    const result = await fraudAnalysisService.execute(createBaseInput());
  ...
  it('13. Deve gravar audit log com a decisão REVIEW', async () => {
    riskEngine.getCountryRisk.mockResolvedValue(75);
  ...
  it('13. Deve gravar audit log com a decisão BLOCKED', async () => {
    riskEngine.getCountryRisk.mockResolvedValue(80);

    const result = await fraudAnalysisService.execute(
      createBaseInput({ amount: 400 })
    );
